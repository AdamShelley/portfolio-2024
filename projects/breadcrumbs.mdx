...
> A breadcrumb component that can be copied into any React or Next app. The breadcrumbs can be built based on the users URL or manually if you want to really customise.

<MarkdownButtons github="https://github.com/AdamShelley/adui/tree/main/packages/Breadcrumb"/>
...
<span className="ml-2 bg-[#eeeeee] p-1 px-3 text-black rounded-sm">@/components/breadcrumbs.tsx</span>
...
- **`mode`** `"url" | "custom"` — Build crumbs from the URL or supply them manually
- **`items`** `BreadcrumbItem[]` — Required when `mode="custom"`
- **`separator`** `"chevron" | "slash" | "line" | "custom"` — Separator style *(default: `"chevron"`)*
- **`customSeparator`** `ReactNode` — Custom separator element when `separator="custom"`
- **`showHome`** `boolean` — Show a home icon at the start *(default: `true`)*
- **`homeLabel`** `string` — Label next to the home icon
- **`homeHref`** `string` — href for the home link *(default: `"/"`)*
- **`variant`** `"default" | "pills" | "bordered"` — Visual style *(default: `"default"`)*
- **`maxItems`** `number` — Max visible crumbs before collapsing into `...` *(default: `5`)*
- **`collapsible`** `boolean` — Allow the breadcrumb to be collapsed *(default: `false`)*
- **`noAnimations`** `boolean` — Disable animations (respects `prefers-reduced-motion` by default)
- **`onItemClick`** `(item, index) => void` — Click handler for crumb items
- **`navClassName`** `string` — Class name for the `<nav>` wrapper
- **`crumbClassNames`** `string` — Class name for individual crumb items